...
```
### Visualiza os containers rodando:
```dockerfile
docker ps
```
...
```
### Parando TODOS os containers que estão rodando:
```dockerfile
docker stop $(docker ps -a -q)
```
### Dando um start em um container parado:
```dockerfile
...
```
### Executando um comando dentro de um container que já está rodando:
```dockerfile
docker exec [OPTIONS] CONTAINER COMMAND [ARG...]
```
### Vendo as imagens que você tem instaladas:
```dockerfile
docker images
```
### Apagando uma imagem:
...
docker rm id_do_container
```
### Fazendo o Build das imagens usando o Docker Compose:
```dockerfile
docker-compose build
```
### Criando e subindo os containers via Docker Compose:
```dockerfile
docker-compose up
```
### Parando os containers via Docker Compose:
```dockerfile
docker-compose stop
```
### Executando um comando dentro de um container via Docker Compose:
```dockerfile
docker-compose run namo_do_service comando
```
### Vendo a sua configuração do Docker Compose:
```dockerfile
docker-compose config
```
### Vendo os logs dos containers via Docker Compose:
```dockerfile
docker-compose logs
```
### Matando os containers via Docker Compose:
```dockerfile
docker-compose kill
```
### Escalando seus containers (Aumentando a quantidade de containers para o mesmo service) via Docker Compose:

```dockerfile
docker-compose scale [SERVICE=NUM...]
```

### Parando e removendo containers usando Docker Compose:

```dockerfile
docker-compose down
```

### Startando os containers:
...
root@0xmachine:/tmp/docker# docker run -d -p 3000:3000 my_awesome_app
```